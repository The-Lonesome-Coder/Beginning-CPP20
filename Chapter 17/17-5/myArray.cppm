...
class MyArray
{
    public:
        ...
        MyArray(MyArray&& moved) noexcept;                  // Move constructor

        ~MyArray();                                         // Destructor

        MyArray& operator = (const MyArray& another);       // Copy assignment
        MyArray& operator = (MyArray&& another) noexcept;   // Move assignment
...
// Move constructor
template <typename T>
MyArray<T>::MyArray(MyArray&& moved) noexcept
...
// Move assignment - Move-and-swap idiom
template <typename T>
MyArray<T>& MyArray<T>::operator = (MyArray&& another) noexcept
{
    MyArray<T> moved(std::move(another));
    swap(moved);
...
{
    push_back(T{ newElement });
}


// Add new element with moving the new element
template <typename T>
void MyArray<T>::push_back(T&& newElement)
{
    ...
    copy[m_size] = std::move(newElement);
